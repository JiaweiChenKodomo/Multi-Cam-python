...
// Modified UI so that it can take commandline variables as instructions directly.
// Need to figure out UILock error later. Currently it is not affecting functionality.
// Jiawei Chen. UC Berkeley, 2024.
...
int main(int argc, char* argv[])
{
	...
	EdsUInt32 camNo = 0;
	EdsUInt32 instrNo = 40;
	std::smatch match_results;
	std::shared_ptr<std::thread> th = std::shared_ptr<std::thread>();

	if (argc >= 3)
	{
		isContinue = false; // In this case, only do what is ordered in the command line.
		camNo = atoi(argv[1]);
		instrNo = atoi(argv[2]);
	}
	//isContinue = false; // In this case, only do what is ordered in the command line.
	...
	while (true)
	{
		...
		while (true)
		{
			...
			else if (!isContinue) 
			{
				...
				std::cout << "connecting to [" << camNo << "] ..." << std::endl;
				camNo -= 1;
				_openedCamerasArray.push_back(_detectedCameraArray[camNo]);
				break;
			}
		...
		while (true)
		{
			...
			if (loop) //Run once
			{
				...
				if (isContinue)
				{
					...
					std::cout
						...
						<< "[33] Set Meta Data(XMP) to All Image files " << std::endl;
					std::cout << "-------------------------------" << std::endl;
			...
			{	
				if (instrNo == 3)
				{
					...
					{
						err = _openedCamerasArray[i]->TakePicture(kEdsCameraCommand_ShutterButton_Completely_NonAF);
						//err = _openedCamerasArray[i]->TakePicture(kEdsCameraCommand_ShutterButton_Completely);
						pause_flg = pause_flg | (err != EDS_ERR_OK);
					}
					if (pause_flg)
				...
				else if (instrNo == 4) // Press halfway
				{
					for (i = 0; i < _openedCamerasArray.size(); i++)
						_openedCamerasArray[i]->PressShutter(kEdsCameraCommand_ShutterButton_Halfway);
					//clr_screen();
				...
				else if (instrNo == 5) // Press Completely
				{
					for (i = 0; i < _openedCamerasArray.size(); i++)
						_openedCamerasArray[i]->PressShutter(kEdsCameraCommand_ShutterButton_Completely);
					//clr_screen();
